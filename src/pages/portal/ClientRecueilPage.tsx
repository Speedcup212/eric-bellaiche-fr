...
const objectiveOptions = [
  ['optimisation_fiscale', 'Optimiser la fiscalité'], ['achat_immobilier', 'Financer un achat immobilier'], ['constitution_patrimoine', 'Constituer / développer un patrimoine'], ['epargne_precaution', 'Constituer une épargne de précaution'], ['liquidites_court_terme', 'Conserver des liquidités à court terme'], ['optimisation_rendement', 'Optimiser le rendement de l’épargne'], ['retraite', 'Préparer la retraite'], ['aide_enfants', 'Aider les enfants / proches'], ['protection_conjoint', 'Protéger le conjoint / les proches'], ['transmission', 'Préparer la transmission'], ['revenus_complementaires', 'Générer des revenus complémentaires'], ['autre', 'Autre objectif'],
] as const;
...
export default function ClientRecueilPage() {
  ...
  useEffect(() => {
    void fetchPortalProgress().then(async (progressRows) => {
      ...
      for (const item of objectiveRows) detailMap[item.code_objectif] = { horizon: item.horizon_annees?.toString() ?? '', note: item.commentaire ?? '', labelOther: item.libelle_autre ?? '' };
  ...
  const saveObjectives = async (row: PortalProgress) => {
    ...
    const { error: deleteError } = await supabase.from('objectifs_patrimoniaux').delete().eq('dossier_id', row.dossier_id).eq('portee', row.role_dossier); if (deleteError) throw deleteError;
    const { error } = await supabase.from('objectifs_patrimoniaux').insert(objectives.map((code) => ({ dossier_id: row.dossier_id, portee: row.role_dossier, code_objectif: code, libelle_autre: code === 'autre' ? objectiveDetails[code]?.labelOther.trim() || null : null, horizon_annees: Number(objectiveDetails[code]?.horizon), commentaire: objectiveDetails[code]?.note.trim() || null }))); if (error) throw error;
  };
  const saveProfessional = async (row: PortalProgress) => {
    if (!profession.trim() || !statutPro.trim()) throw new Error('Renseignez votre profession actuelle et votre statut pour continuer.');
    const { error } = await supabase.from('situations_professionnelles').upsert({ dossier_id: row.dossier_id, investisseur_id: row.investisseur_id, profession_actuelle: profession.trim(), societe: societe.trim() || null, secteur_activite: secteur.trim() || null, statut: statutPro.trim() }, { onConflict: 'dossier_id,investisseur_id' }); if (error) throw error;
  };
  const saveCapacity = async (row: PortalProgress) => {
    if (epargneMensuelle === '' || epargnePrecaution === '') throw new Error('Renseignez votre épargne mensuelle disponible et votre épargne de précaution cible. Vous pouvez indiquer 0 si nécessaire.');
    const { error } = await supabase.from('capacites_financieres').upsert({ dossier_id: row.dossier_id, investisseur_id: row.investisseur_id, capacite_epargne_mensuelle: Number(epargneMensuelle), epargne_precaution_cible: Number(epargnePrecaution), apport_immobilier_possible: apport ? Number(apport) : null }, { onConflict: 'dossier_id,investisseur_id' }); if (error) throw error;
  };
  const saveEsg = async (row: PortalProgress) => { if (esg === null) throw new Error('Indiquez si vous souhaitez exprimer des préférences de durabilité.'); const { error } = await supabase.rpc('set_my_esg_opt_in', { p_dossier_id: row.dossier_id, p_opt_in: esg }); if (error) throw error; };
  const saveCurrent = async () => { if (!progress) return; if (stepIndex === 0) await saveObjectives(progress); if (stepIndex === 1) await saveProfessional(progress); if (stepIndex === 2) await saveCapacity(progress); if (stepIndex === 3) await saveEsg(progress); };
  ...
  const next = async () => {
    ...
    try {
      await saveCurrent();
      if (stepIndex < steps.length - 1) { setStepIndex((value) => value + 1); window.scrollTo({ top: 0, behavior: 'smooth' }); return; }
      const { error: validateError } = await supabase.rpc('validate_my_recueil', { p_dossier_id: progress.dossier_id }); if (validateError) throw validateError;
      const refreshed = await fetchPortalProgress(); setRows(refreshed); const nextProgress = selectedProgress(refreshed, progress.dossier_id);
  ...
  return <div>
    ...
    <WizardCard>
      ...
      <div className="px-6 py-7 sm:px-9 sm:py-9">
        {stepIndex === 0 && <div className="space-y-6"><div className="grid gap-3 sm:grid-cols-2">{objectiveOptions.map(([code, label]) => { const selected = objectives.includes(code); return <button type="button" key={code} onClick={() => toggleObjective(code)} className={`flex items-center gap-3 rounded-2xl border p-4 text-left text-sm font-semibold transition ${selected ? 'border-slate-950 bg-slate-950 text-white shadow-lg shadow-slate-950/10' : 'border-slate-200 bg-white text-slate-700 hover:border-slate-400'}`}><span className={`flex h-6 w-6 items-center justify-center rounded-full border ${selected ? 'border-white/30 bg-white text-slate-950' : 'border-slate-300'}`}>{selected ? '✓' : ''}</span>{label}</button>; })}</div>{objectives.length > 0 && <div className="space-y-4 border-t border-slate-100 pt-6"><div><h4 className="font-semibold text-slate-950">Horizon et précisions</h4><p className="mt-1 text-sm leading-6 text-slate-500">Complétez ces informations pour chaque objectif sélectionné. L’horizon est obligatoire ; la note est facultative.</p></div>{objectives.map((code) => { const label = objectiveOptions.find(([value]) => value === code)?.[1] ?? code; const detail = objectiveDetails[code] ?? { horizon: '', note: '', labelOther: '' }; return <div key={code} className="rounded-2xl border border-slate-200 bg-slate-50/60 p-4 sm:p-5"><p className="font-semibold text-slate-800">{label}</p>{code === 'autre' && <label className="mt-4 block text-sm font-semibold text-slate-700">Précisez l’objectif<input value={detail.labelOther} onChange={(e) => updateObjectiveDetail(code, { labelOther: e.target.value })} className="mt-2 w-full rounded-xl border border-slate-200 bg-white px-4 py-3" placeholder="Ex. financer les études des enfants" /></label>}<div className="mt-4 grid gap-4 sm:grid-cols-[180px_1fr]"><label className="text-sm font-semibold text-slate-700">Horizon (années) *<input type="number" min="0.5" step="0.5" value={detail.horizon} onChange={(e) => updateObjectiveDetail(code, { horizon: e.target.value })} className="mt-2 w-full rounded-xl border border-slate-200 bg-white px-4 py-3" placeholder="Ex. 8" /></label><label className="text-sm font-semibold text-slate-700">Notes / précisions<textarea value={detail.note} onChange={(e) => updateObjectiveDetail(code, { note: e.target.value })} rows={3} className="mt-2 w-full resize-none rounded-xl border border-slate-200 bg-white px-4 py-3" placeholder="Échéance, montant, contexte, priorité…" /></label></div></div>; })}</div>}</div>}
        {stepIndex === 1 && <div className="grid gap-5 sm:grid-cols-2"><label className="text-sm font-semibold text-slate-700">Profession actuelle *<input value={profession} onChange={(e) => setProfession(e.target.value)} className="mt-2 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3.5" placeholder="Ex. Cadre commercial, retraité…" /></label><label className="text-sm font-semibold text-slate-700">Employeur / société<input value={societe} onChange={(e) => setSociete(e.target.value)} className="mt-2 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3.5" /></label><label className="text-sm font-semibold text-slate-700">Secteur d’activité<input value={secteur} onChange={(e) => setSecteur(e.target.value)} className="mt-2 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3.5" /></label><label className="text-sm font-semibold text-slate-700">Statut / contrat *<input value={statutPro} onChange={(e) => setStatutPro(e.target.value)} className="mt-2 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3.5" placeholder="CDI, indépendant, retraité…" /></label></div>}
        {stepIndex === 2 && <div className="space-y-5"><div className="grid gap-5 sm:grid-cols-3"><label className="text-sm font-semibold text-slate-700">Épargne mensuelle disponible (€) *<input type="number" min="0" value={epargneMensuelle} onChange={(e) => setEpargneMensuelle(e.target.value)} className="mt-2 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3.5" /></label><label className="text-sm font-semibold text-slate-700">Épargne de précaution cible (€) *<input type="number" min="0" value={epargnePrecaution} onChange={(e) => setEpargnePrecaution(e.target.value)} className="mt-2 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3.5" /></label><label className="text-sm font-semibold text-slate-700">Apport immobilier possible (€)<input type="number" min="0" value={apport} onChange={(e) => setApport(e.target.value)} className="mt-2 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3.5" /></label></div><SecureNote>Indiquez 0 si le montant est nul. Le cabinet pourra rapprocher ces données des justificatifs transmis et vous demander une précision si nécessaire.</SecureNote></div>}
        {stepIndex === 3 && <div className="space-y-5"><div className="grid gap-4 sm:grid-cols-2"><button type="button" onClick={() => setEsg(true)} className={`rounded-2xl border p-5 text-left transition ${esg === true ? 'border-slate-950 bg-slate-950 text-white shadow-lg' : 'border-slate-200 hover:border-slate-400'}`}><p className="font-semibold">Oui, je souhaite les préciser</p><p className={`mt-2 text-sm leading-6 ${esg === true ? 'text-slate-300' : 'text-slate-500'}`}>Un questionnaire dédié vous sera proposé après le profil investisseur.</p></button><button type="button" onClick={() => setEsg(false)} className={`rounded-2xl border p-5 text-left transition ${esg === false ? 'border-slate-950 bg-slate-950 text-white shadow-lg' : 'border-slate-200 hover:border-slate-400'}`}><p className="font-semibold">Non, je ne souhaite pas en exprimer</p><p className={`mt-2 text-sm leading-6 ${esg === false ? 'text-slate-300' : 'text-slate-500'}`}>Aucun questionnaire de durabilité supplémentaire ne vous sera présenté.</p></button></div><SecureNote>Ce choix ne détermine pas votre profil de risque. Il concerne uniquement la prise en compte de préférences de durabilité dans les solutions étudiées.</SecureNote></div>}
