              ...
              <div className="bg-gradient-to-br from-[#071a33] via-[#0b1f3a] to-[#173967] px-6 py-7 text-white sm:px-9 sm:py-9">
                <div className="flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
                  <div className="max-w-2xl">
                    ...
                    <h2 className="mt-3 text-2xl font-semibold tracking-tight sm:text-3xl">{allDone ? 'Votre dossier est transmis' : coupleWaiting ? 'Votre parcours est terminé à 100 %' : 'Reprenez votre parcours'}</h2>
                    <p className="mt-3 text-sm leading-6 text-blue-100/80">
                      {allDone ? 'Le cabinet dispose maintenant des éléments transmis pour poursuivre l’analyse.' : coupleWaiting ? <>Votre partie est complète. {row.partner_name ? <>Il reste à <span className="font-semibold text-white">{row.partner_name}</span> de terminer son parcours.</> : <>Il reste à l’autre personne de terminer son parcours.</>} Vous pouvez déjà déposer les justificatifs communs.</> : <>Étape actuelle : <span className="font-semibold text-white">{currentStepTitle(row.next_step)}</span>. Vos réponses sont enregistrées au fur et à mesure.</>}
                    </p>
                  </div>
                  {!allDone ? (
            ...
            {row.is_couple && !row.dossier_ready_for_documents && !allDone && (
              <div className="rounded-[22px] border border-amber-200 bg-amber-50 px-5 py-4 shadow-sm sm:px-6">
                <div className="flex items-start gap-4">
                  ...
                  <div><h3 className="font-semibold text-amber-950">{individualDone ? 'Votre parcours : 100 % terminé' : 'Dossier couple : parcours en cours'}</h3><p className="mt-1.5 text-sm leading-6 text-amber-800">{individualDone ? <>{row.partner_name ? <>Il reste maintenant à <strong>{row.partner_name}</strong> de compléter son parcours individuel et de confirmer les informations communes du foyer.</> : <>Il reste maintenant à l’autre personne de compléter son parcours individuel et de confirmer les informations communes du foyer.</>}</> : <>Les informations communes du foyer sont saisies une seule fois puis vérifiées par le second membre. Chacun conserve ses informations personnelles, son profil investisseur et ses préférences de durabilité.</>}</p>{!row.partner_activated && <p className="mt-2 text-sm font-semibold text-amber-900">{row.partner_name ? <>{row.partner_name} n’a pas encore activé son accès sécurisé.</> : <>L’autre personne n’a pas encore activé son accès sécurisé.</>}</p>}</div>
                </div>
              </div>
            )}
