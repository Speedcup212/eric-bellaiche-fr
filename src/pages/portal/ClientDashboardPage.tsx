...
export default function ClientDashboardPage() {
  ...
  return (
    <div>
      ...
      {rows.length === 0 ? (
        <WizardCard className="p-8"><ShieldCheck className="h-8 w-8 text-slate-400" /><h3 className="mt-4 text-xl font-semibold">Aucun dossier rattaché</h3><p className="mt-2 text-sm leading-6 text-slate-500">Votre accès est actif mais aucun dossier n’est encore rattaché. Utilisez le lien personnel transmis par le cabinet.</p></WizardCard>
      ) : rows.map((row) => {
            ...
            <WizardCard>
              <div className="bg-gradient-to-br from-slate-950 via-slate-900 to-indigo-950 px-6 py-7 text-white sm:px-9 sm:py-9">
                <div className="flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
                  <div><p className="text-xs font-bold uppercase tracking-[0.18em] text-cyan-300">{row.reference || 'Dossier patrimonial'}</p><h3 className="mt-2 text-2xl font-semibold tracking-tight">{row.libelle || 'Accompagnement patrimonial'}</h3><p className="mt-2 text-sm text-slate-300">{docs} document{docs === 1 ? '' : 's'} transmis · réponses sauvegardées automatiquement</p></div>
                  {!allDone ? <Link to={nextStepHref(row)} className="inline-flex items-center justify-center gap-2 rounded-xl bg-white px-5 py-3 text-sm font-semibold text-slate-950 shadow-xl shadow-black/10 transition hover:-translate-y-0.5">{stepLabel(row.next_step)} <ArrowRight className="h-4 w-4" /></Link> : <span className="inline-flex items-center gap-2 rounded-xl bg-emerald-400/15 px-4 py-3 text-sm font-semibold text-emerald-200 ring-1 ring-inset ring-emerald-300/20"><CheckCircle2 className="h-4 w-4" /> Dossier transmis</span>}
                </div>
              </div>
