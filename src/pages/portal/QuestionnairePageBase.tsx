  ...
  return <div>
    ...
    <WizardCard>
      ...
      <div className="px-6 py-7 sm:px-9 sm:py-9">
        ...
        {detailStep === 0 && <div className="space-y-5"><div className="grid gap-3 sm:grid-cols-2"><ChoiceButton selected={expDetails.connaissance === 'true'} onClick={() => { const nextState = { ...expDetails, connaissance: 'true' as const }; setExpDetails(nextState); void saveExperienceDetails(nextState); }}>Oui</ChoiceButton><ChoiceButton selected={expDetails.connaissance === 'false'} onClick={() => { const nextState = { ...expDetails, connaissance: 'false' as const, sources: [], precision: '' }; setExpDetails(nextState); void saveExperienceDetails(nextState).then(() => { setCurrentIndex((index) => Math.min(index + 1, totalSteps - 1)); window.scrollTo({ top: 0, behavior: 'smooth' }); }).catch((error) => setErrorMessage(messageFromError(error))); }}>Non</ChoiceButton></div>{expDetails.connaissance === 'true' && <div className="rounded-2xl border border-slate-200 bg-slate-50/70 p-5"><p className="font-semibold text-slate-800">Origine de vos connaissances</p><p className="mt-1 text-sm leading-6 text-slate-500">Sélectionnez au moins une source ou précisez-la librement.</p><div className="mt-4 grid gap-3 sm:grid-cols-2">{knowledgeSources.map(([code, label]) => { const selected = expDetails.sources.includes(code); return <button key={code} type="button" onClick={() => setExpDetails((state) => ({ ...state, sources: selected ? state.sources.filter((value) => value !== code) : [...state.sources, code] }))} className={`rounded-xl border px-4 py-3 text-left text-sm ${selected ? 'border-slate-950 bg-slate-950 text-white' : 'border-slate-200 bg-white'}`}>{selected ? '✓ ' : ''}{label}</button>; })}</div><textarea value={expDetails.precision} onChange={(e) => setExpDetails((state) => ({ ...state, precision: e.target.value }))} rows={3} className="mt-4 w-full resize-none rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm" placeholder="Précision facultative…" /></div>}</div>}
        {detailStep === 1 && <div className="grid gap-3">{seniorityOptions.map(([value, label]) => <ChoiceButton key={value} selected={expDetails.anciennete === value} onClick={() => { const nextState = { ...expDetails, anciennete: value }; setExpDetails(nextState); void saveExperienceDetails(nextState).then(() => { setCurrentIndex((index) => Math.min(index + 1, totalSteps - 1)); window.scrollTo({ top: 0, behavior: 'smooth' }); }).catch((error) => setErrorMessage(messageFromError(error))); }}>{label}</ChoiceButton>)}</div>}
        {detailStep === 2 && <div className="grid gap-3">{amountOptions.map(([value, label]) => <ChoiceButton key={value} selected={expDetails.montant === value} onClick={() => { const nextState = { ...expDetails, montant: value }; setExpDetails(nextState); void saveExperienceDetails(nextState).then(() => { setCurrentIndex((index) => Math.min(index + 1, totalSteps - 1)); window.scrollTo({ top: 0, behavior: 'smooth' }); }).catch((error) => setErrorMessage(messageFromError(error))); }}>{label}</ChoiceButton>)}</div>}
        {detailStep === 3 && <div className="grid gap-3">{managementOptions.map(([value, label]) => <ChoiceButton key={value} selected={expDetails.mode === value} onClick={() => { const nextState = { ...expDetails, mode: value }; setExpDetails(nextState); void finish(nextState).catch((error) => setErrorMessage(messageFromError(error))); }}>{label}</ChoiceButton>)}</div>}
      ...
      {currentQuestion?.type_reponse === 'single' || detailStep === 1 || detailStep === 2 || detailStep === 3 || (detailStep === 0 && expDetails.connaissance === 'false') ? (
