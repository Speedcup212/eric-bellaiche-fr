...
function financialSnapshot(sections: SectionRow[], householdRealEstate: number) {
  ...
  return { annualIncome, annualCharges, savingsCapacityMonthly, bankAvailable, financialAssets, monthlyDebt, debtOutstanding, tmi, taxableIncome, rfr, incomeTax, perCeiling, ifi, landDeficit, familyStatus, matrimonialRegime, children, notary, transmissionClause, debtRatio, remainingMonthly, netRealEstate, patrimonyNet, goals, transmissionGoals, missing };
}
...
export default function CifDossierSummaryPage() {
  ...
  return <div className="min-h-screen bg-blue-50/30 px-4 py-8 sm:px-6"><div className="mx-auto max-w-7xl space-y-6">
    ...
    <section className="rounded-3xl border border-blue-100 bg-white p-6 shadow-sm sm:p-8"><div className="flex items-start gap-3"><div className="rounded-2xl bg-blue-50 p-3"><Home className="h-5 w-5 text-blue-700" /></div><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-blue-600">Photographie patrimoniale du foyer</p><h2 className="mt-1 text-xl font-semibold text-slate-950">Les chiffres utiles pour travailler le dossier</h2><p className="mt-1 text-sm text-slate-500">Lecture par problématique patrimoniale. Aucune valeur n’est déduite lorsqu’une donnée nécessaire manque.</p></div></div>
      ...
      <div className="mt-5 grid gap-3 lg:grid-cols-2"><div className="rounded-2xl border border-[#25405F] bg-[#0B1A2F] p-4 text-sm text-slate-200"><strong className="text-white">Objectifs prioritaires :</strong> {snapshot.goals.length ? snapshot.goals.join(' · ') : <span className="text-amber-300">À préciser</span>}</div><div className={`rounded-2xl border p-4 text-sm ${snapshot.missing.length ? 'border-amber-500/60 bg-amber-950/20 text-amber-100' : 'border-emerald-500/40 bg-emerald-950/20 text-emerald-100'}`}><strong>{snapshot.missing.length ? 'Points à compléter avant conseil :' : 'Données de travail principales disponibles.'}</strong>{snapshot.missing.length ? ` ${snapshot.missing.join(', ')}.` : ''}</div></div>
