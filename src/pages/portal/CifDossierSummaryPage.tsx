  ...
  return <div className="min-h-screen bg-blue-50/30 px-4 py-8 sm:px-6"><div className="mx-auto max-w-7xl space-y-6">
    ...
    {activeTab === 'documents' && <section className="rounded-3xl border border-emerald-100 bg-white p-6 shadow-sm sm:p-8"><div className="flex items-start gap-3"><div className="rounded-2xl bg-emerald-50 p-3"><FileText className="h-5 w-5 text-emerald-700" /></div><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-emerald-700">Documents du dossier</p><h2 className="mt-1 text-xl font-semibold text-slate-950">Collecte client et chaîne réglementaire</h2><p className="mt-1 text-sm text-slate-500">Les documents sont regroupés selon leur rôle dans le parcours : collecte, justificatifs reçus, mission puis adéquation du conseil.</p></div></div>{sourceAnalysisMessage && <p className="mt-5 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">{sourceAnalysisMessage}</p>}<div className="mt-6 grid gap-5 lg:grid-cols-2"><div><p className="text-xs font-bold uppercase tracking-[0.12em] text-slate-500">Collecte client</p><div className="mt-3 space-y-3">{investorDocumentStates.map((state) => { const investor = state.investor; const name = `${investor.investisseurs?.prenom ?? ''} ${investor.investisseurs?.nom ?? ''}`.trim() || 'Client'; const individualDone = state.recueil && state.qpi && (state.esg || state.esgNotApplicable); return <div key={investor.investisseur_id} className={`relative overflow-hidden rounded-2xl border p-4 pl-5 shadow-[0_10px_28px_rgba(15,23,42,0.07)] ${individualDone ? 'border-emerald-200/80 bg-gradient-to-r from-emerald-50/95 via-white to-white' : 'border-blue-200/80 bg-gradient-to-r from-blue-50/95 via-white to-white'}`}><span aria-hidden="true" className={`absolute bottom-4 left-0 top-4 w-1 rounded-r-full ${individualDone ? 'bg-emerald-400' : 'bg-blue-400'}`} /><div className="flex flex-wrap items-center justify-between gap-3"><div><p className={`font-semibold ${individualDone ? 'text-emerald-950' : 'text-blue-950'}`}>{name}</p><p className={`mt-0.5 text-xs ${individualDone ? 'text-emerald-700/70' : 'text-blue-700/70'}`}>{investor.role_dossier === 'investisseur_1' ? 'Identifiant 1' : 'Identifiant 2'}</p></div><span className={`rounded-full border px-2.5 py-1 text-[10px] font-bold uppercase shadow-sm ${individualDone ? 'border-emerald-200 bg-emerald-100/80 text-emerald-700' : 'border-blue-200 bg-blue-100/80 text-blue-700'}`}>{individualDone ? 'Parcours terminé' : 'Parcours en cours'}</span></div><div className="mt-3 grid min-w-0 gap-3 sm:grid-cols-3">{(['recueil','qpi','esg'] as GeneratedDocument['type'][]).map((type) => {
          ...
          const typeTheme = type === 'recueil'
            ? { card:'border-blue-200 bg-gradient-to-br from-blue-50 via-white to-blue-50/40', title:'text-blue-950', badgeDone:'bg-blue-700 text-white', action:'bg-blue-700 hover:bg-blue-800', soft:'border-blue-200 text-blue-800 hover:bg-blue-50' }
            : type === 'qpi'
          ...
          const cardTitle = type === 'recueil' ? 'Recueil' : type === 'qpi' ? 'Profil investisseur' : 'Durabilité';
          const cardSubtitle = type === 'recueil' ? 'Informations client' : type === 'qpi' ? 'Questionnaire de profil' : 'Préférences ESG';
          ...
          return <div key={type} className={`flex min-w-0 flex-col rounded-2xl border p-4 shadow-[0_6px_18px_rgba(15,23,42,0.05)] ${classes}`}>
            <div className="min-w-0">
              <p className={`break-words text-[14px] font-extrabold leading-[1.15] tracking-[-0.01em] ${typeTheme.title}`}>{cardTitle}</p>
              <p className="mt-1 break-words text-[10px] font-medium leading-4 text-slate-500">{cardSubtitle}</p>
              <div className="mt-2">
                <span className={`inline-flex max-w-full items-center rounded-full px-2 py-1 text-[9px] font-extrabold uppercase leading-none tracking-[0.04em] whitespace-nowrap ${badge}`}>{status}</span>
              </div>
              {notApplicable && <p className="mt-2 text-[10px] font-medium leading-4 text-teal-700">Aucune préférence exprimée.</p>}
            </div>
            {(completed || showRecueilActions) && <div className="mt-auto space-y-2 pt-4">
              {document?.signed_url ? <a href={document.signed_url} target="_blank" rel="noreferrer" className={`inline-flex w-full items-center justify-center gap-1.5 rounded-lg px-2.5 py-2 text-[10px] font-bold text-white shadow-sm transition ${completed ? typeTheme.action : 'bg-slate-950 hover:bg-slate-800'}`}><Download className="h-3.5 w-3.5 shrink-0" /><span className="whitespace-nowrap">Voir le PDF</span></a> : generatingDocuments ? <span className="inline-flex w-full items-center justify-center gap-1.5 text-[10px] font-semibold text-blue-700"><Loader2 className="h-3.5 w-3.5 animate-spin" /> Préparation</span> : null}
              {type === 'recueil' && <button type="button" onClick={() => setActiveTab('clients')} className={`inline-flex w-full items-center justify-center rounded-lg border bg-white px-2.5 py-2 text-[10px] font-bold transition ${typeTheme.soft}`}><span className="whitespace-nowrap">Voir les réponses</span></button>}
            </div>}
