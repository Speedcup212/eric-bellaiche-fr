            ...
            <div className="rounded-2xl border border-[#25405F] bg-[#0B1A2F] p-4 shadow-[0_14px_34px_rgba(2,10,25,0.18)] sm:p-5">
              ...
              <div className="mt-4 space-y-2">
                <div className={`rounded-xl border px-3.5 py-3 ${householdRecueilState.complete ? 'border-blue-500/40 bg-[#102A4C]' : 'border-amber-500/40 bg-[#3A2A0A]'}`}>
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {recueilDocument?.signed_url ? <a href={recueilDocument.signed_url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 rounded-lg bg-blue-600 px-3 py-2 text-[10px] font-bold text-white transition hover:bg-blue-500"><Download className="h-3.5 w-3.5" /> {isCoupleDossier ? 'Voir le PDF commun' : 'Voir le PDF'}</a> : generatingDocuments ? <span className="inline-flex items-center gap-1.5 text-[10px] font-semibold text-blue-200"><Loader2 className="h-3.5 w-3.5 animate-spin" /> {isCoupleDossier ? 'Préparation du recueil commun' : 'Préparation du recueil'}</span> : null}
