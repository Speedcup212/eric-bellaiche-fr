...
function BoolChoice({ label, value, onChange }: { label: string; value: boolean | ''; onChange: (value: boolean) => void }) {
  return <div><p className="text-sm font-semibold text-slate-700">{label}</p><div className="mt-2 grid grid-cols-2 gap-2">{[[true, 'Oui'], [false, 'Non']].map(([v, text]) => <button key={text as string} type="button" onClick={() => onChange(v as boolean)} className={`rounded-xl border px-4 py-3 text-sm font-semibold ${value === v ? 'border-slate-950 bg-slate-950 text-white' : 'border-slate-200 bg-white text-slate-700'}`}>{text as string}</button>)}</div></div>;
}
...
export default function ClientRecueilJourneyPage() {
  ...
  const validateSection = () => {
    if (current.code === 'identity') {
      if (!form.prenom?.trim() || !form.nom?.trim() || !form.date_naissance) throw new Error('Renseignez votre prénom, votre nom et votre date de naissance.');
      if (!form.address?.code_postal?.trim() || !form.address?.ville?.trim()) throw new Error('Renseignez votre code postal et votre ville.');
    }
    if (current.code === 'family' && !form.situation?.trim()) throw new Error('Renseignez votre situation familiale.');
    if (current.code === 'professional' && (!form.profession_actuelle?.trim() || !form.statut?.trim())) throw new Error('Renseignez votre profession actuelle et votre statut.');
    if (current.code === 'objectives') {
      ...
      for (const item of form.items) {
        if (item.montant_cible === '' || item.horizon_annees === '' || Number(item.horizon_annees) < 0) throw new Error('Indiquez un montant cible et un horizon pour chaque objectif.');
        if (item.code_objectif === 'autre' && !item.libelle_autre?.trim()) throw new Error('Précisez votre autre objectif.');
      }
    }
    if (current.code === 'capacity' && (form.epargne_precaution_cible === '' || form.capacite_epargne_mensuelle === '')) throw new Error('Renseignez votre épargne de précaution cible et votre capacité d’épargne mensuelle.');
    if (current.code === 'tax' && !form.annee_imposition) throw new Error('Indiquez l’année d’imposition.');
    if (current.code === 'regulatory') {
      if (!form.pays_residence_fiscale?.trim()) throw new Error('Indiquez votre pays de résidence fiscale.');
  ...
  const next = async () => {
    ...
    try {
      ...
      if (error) throw error;
      const refreshed = await fetchPortalProgress(); setRows(refreshed);
  ...
  return <div>
    ...
    <WizardCard>
      ...
      <div className="space-y-6 px-6 py-7 sm:px-9 sm:py-9">
        {current.code === 'identity' && <><div className="grid gap-4 sm:grid-cols-3"><Field label="Civilité" value={form.civilite} onChange={(v) => patchCurrent({ civilite: v })} /><Field label="Prénom" required value={form.prenom} onChange={(v) => patchCurrent({ prenom: v })} /><Field label="Nom" required value={form.nom} onChange={(v) => patchCurrent({ nom: v })} /><Field label="Nom de naissance" value={form.nom_naissance} onChange={(v) => patchCurrent({ nom_naissance: v })} /><Field label="Date de naissance" required type="date" value={form.date_naissance} onChange={(v) => patchCurrent({ date_naissance: v })} /><Field label="Lieu de naissance" value={form.lieu_naissance} onChange={(v) => patchCurrent({ lieu_naissance: v })} /><Field label="Pays de naissance" value={form.pays_naissance} onChange={(v) => patchCurrent({ pays_naissance: v })} /><Field label="Nationalité" value={form.nationalite} onChange={(v) => patchCurrent({ nationalite: v })} /><Field label="Mobile" value={form.mobile} onChange={(v) => patchCurrent({ mobile: v })} /></div><div className="border-t border-slate-100 pt-5"><h3 className="font-semibold text-slate-900">Adresse fiscale</h3><div className="mt-4 grid gap-4 sm:grid-cols-2"><Field label="N° et voie" value={form.address?.numero_voie} onChange={(v) => patchCurrent({ address: { ...form.address, numero_voie: v } })} /><Field label="Complément" value={form.address?.complement} onChange={(v) => patchCurrent({ address: { ...form.address, complement: v } })} /><Field label="Code postal" required value={form.address?.code_postal} onChange={(v) => patchCurrent({ address: { ...form.address, code_postal: v } })} /><Field label="Ville" required value={form.address?.ville} onChange={(v) => patchCurrent({ address: { ...form.address, ville: v } })} /><Field label="Pays" value={form.address?.pays} onChange={(v) => patchCurrent({ address: { ...form.address, pays: v } })} /><Field label="Type de logement" value={form.address?.type_logement} onChange={(v) => patchCurrent({ address: { ...form.address, type_logement: v } })} /></div></div></>}

        {current.code === 'family' && <div className="grid gap-4 sm:grid-cols-2"><Field label="Situation familiale" required value={form.situation} onChange={(v) => patchCurrent({ situation: v })} placeholder="Autre situation" /><Field label="Date mariage / PACS / divorce" type="date" value={form.date_evenement} onChange={(v) => patchCurrent({ date_evenement: v })} /><Field label="Régime / convention" value={form.regime_convention} onChange={(v) => patchCurrent({ regime_convention: v })} /><Field label="Avantage matrimonial" value={form.avantage_matrimonial} onChange={(v) => patchCurrent({ avantage_matrimonial: v })} /><Field label="Nombre d’enfants" type="number" value={form.nombre_enfants} onChange={(v) => patchCurrent({ nombre_enfants: v })} /><Field label="Notaire + ville" value={form.notaire_nom_ville} onChange={(v) => patchCurrent({ notaire_nom_ville: v })} /><Field label="Expert-comptable + ville" value={form.expert_comptable_nom_ville} onChange={(v) => patchCurrent({ expert_comptable_nom_ville: v })} /><Field label="Évolution prévue" value={form.evolution_prevue} onChange={(v) => patchCurrent({ evolution_prevue: v })} /></div>}

        {current.code === 'professional' && <div className="grid gap-4 sm:grid-cols-2"><Field label="Profession actuelle" required value={form.profession_actuelle} onChange={(v) => patchCurrent({ profession_actuelle: v })} /><Field label="Société / employeur" value={form.societe} onChange={(v) => patchCurrent({ societe: v })} /><Field label="Secteur d’activité" value={form.secteur_activite} onChange={(v) => patchCurrent({ secteur_activite: v })} /><Field label="Statut" required value={form.statut} onChange={(v) => patchCurrent({ statut: v })} placeholder="Autre statut" /><Field label="Catégorie socioprofessionnelle" value={form.categorie_socioprofessionnelle} onChange={(v) => patchCurrent({ categorie_socioprofessionnelle: v })} /><Field label="Ancienneté (années)" type="number" value={form.anciennete_annees} onChange={(v) => patchCurrent({ anciennete_annees: v })} /><Field label="Date d’entrée" type="date" value={form.date_entree} onChange={(v) => patchCurrent({ date_entree: v })} /><Field label="Origine des revenus si sans activité" value={form.origine_revenus_sans_activite} onChange={(v) => patchCurrent({ origine_revenus_sans_activite: v })} /></div>}

        {current.code === 'objectives' && <><div className="grid gap-3 sm:grid-cols-2">{objectiveOptions.map(([code, label]) => { const selected = objectiveItems.some((item) => item.code_objectif === code); return <button type="button" key={code} onClick={() => toggleObjective(code, label)} className={`rounded-xl border p-4 text-left text-sm font-semibold ${selected ? 'border-slate-950 bg-slate-950 text-white' : 'border-slate-200 bg-white text-slate-700'}`}>{selected ? '✓ ' : ''}{label}</button>; })}</div>{objectiveItems.map((item) => <div key={item.code_objectif} className="rounded-2xl border border-slate-200 bg-slate-50 p-5"><p className="font-semibold text-slate-900">{item.label || objectiveOptions.find(([code]) => code === item.code_objectif)?.[1]}</p>{item.code_objectif === 'autre' && <div className="mt-4"><Field label="Précisez l’objectif" required value={item.libelle_autre} onChange={(v) => updateObjective(item.code_objectif, { libelle_autre: v })} /></div>}<div className="mt-4 grid gap-4 sm:grid-cols-2"><MoneyField label="Montant cible (€)" required value={item.montant_cible} onChange={(v) => updateObjective(item.code_objectif, { montant_cible: v })} /><Field label="Horizon (années)" required type="number" value={item.horizon_annees} onChange={(v) => updateObjective(item.code_objectif, { horizon_annees: v })} /></div><div className="mt-4"><Field label="Précisions" value={item.commentaire} onChange={(v) => updateObjective(item.code_objectif, { commentaire: v })} /></div></div>)}</>}

        {current.code === 'capacity' && <div className="grid gap-4 sm:grid-cols-2"><MoneyField label="Revenus du travail estimés annuels (€)" value={form.estimation_revenus_travail_annuels} onChange={(v) => patchCurrent({ estimation_revenus_travail_annuels: v })} /><MoneyField label="Revenus fonciers estimés annuels (€)" value={form.estimation_revenus_fonciers_annuels} onChange={(v) => patchCurrent({ estimation_revenus_fonciers_annuels: v })} /><MoneyField label="Épargne de précaution cible (€)" required value={form.epargne_precaution_cible} onChange={(v) => patchCurrent({ epargne_precaution_cible: v })} /><MoneyField label="Capacité d’épargne mensuelle (€)" required value={form.capacite_epargne_mensuelle} onChange={(v) => patchCurrent({ capacite_epargne_mensuelle: v })} /><MoneyField label="Apport immobilier possible (€)" value={form.apport_immobilier_possible} onChange={(v) => patchCurrent({ apport_immobilier_possible: v })} /></div>}

        {current.code === 'tax' && <><div className="grid gap-4 sm:grid-cols-3"><Field label="Année d’imposition" required type="number" value={form.annee_imposition} onChange={(v) => patchCurrent({ annee_imposition: v })} /><MoneyField label="Revenu imposable (€)" value={form.revenu_imposable} onChange={(v) => patchCurrent({ revenu_imposable: v })} /><MoneyField label="Revenu fiscal de référence (€)" value={form.revenu_fiscal_reference} onChange={(v) => patchCurrent({ revenu_fiscal_reference: v })} /><Field label="Nombre de parts" type="number" value={form.nombre_parts} onChange={(v) => patchCurrent({ nombre_parts: v })} /><Field label="TMI (%)" type="number" value={form.tmi} onChange={(v) => patchCurrent({ tmi: v })} /><MoneyField label="Impôt sur le revenu net (€)" value={form.impot_revenu_net} onChange={(v) => patchCurrent({ impot_revenu_net: v })} /><MoneyField label="Salaires / assimilés (€)" value={form.salaires_assimiles} onChange={(v) => patchCurrent({ salaires_assimiles: v })} /><MoneyField label="Revenus fonciers nets (€)" value={form.revenus_fonciers_nets} onChange={(v) => patchCurrent({ revenus_fonciers_nets: v })} /><MoneyField label="Déficit foncier reportable (€)" value={form.deficit_foncier_reportable} onChange={(v) => patchCurrent({ deficit_foncier_reportable: v })} /><MoneyField label="Plafond épargne retraite disponible (€)" value={form.plafond_disponible_avis} onChange={(v) => patchCurrent({ plafond_disponible_avis: v })} /><MoneyField label="Versements retraite à déduire (€)" value={form.versements_a_deduire} onChange={(v) => patchCurrent({ versements_a_deduire: v })} /></div><div className="border-t border-slate-100 pt-5"><BoolChoice label="Êtes-vous concerné par l’IFI ?" value={form.ifi_concerne} onChange={(v) => patchCurrent({ ifi_concerne: v })} />{form.ifi_concerne && <div className="mt-4 grid gap-4 sm:grid-cols-3"><MoneyField label="Base imposable IFI (€)" value={form.ifi_base_imposable} onChange={(v) => patchCurrent({ ifi_base_imposable: v })} /><Field label="TMI IFI (%)" type="number" value={form.ifi_tmi} onChange={(v) => patchCurrent({ ifi_tmi: v })} /><MoneyField label="IFI net à payer (€)" value={form.ifi_net_a_payer} onChange={(v) => patchCurrent({ ifi_net_a_payer: v })} /></div>}</div></>}
