    ...
    <WizardCard>
      ...
      <div className="space-y-10 px-6 py-9 sm:px-9 sm:py-12">
        ...
        {current.code === 'regulatory' && <div className="space-y-8 [&>*]:border-b [&>*]:border-white/15 [&>*]:pb-5">
          ...
          {form.ppe_declaree === true && <div className="grid gap-x-5 gap-y-7 sm:grid-cols-2 sm:gap-x-6 sm:gap-y-8 [&>*]:border-b [&>*]:border-white/15 [&>*]:pb-4"><Field label="Personne concernée" required value={form.ppe_personne_exposee} onChange={(v) => patchCurrent({ ppe_personne_exposee: v })} placeholder="Vous-même ou nom du proche" /><Field label="Fonction exercée" required value={form.ppe_motif} onChange={(v) => patchCurrent({ ppe_motif: v })} placeholder="Ex. député, ambassadeur, dirigeant public…" /><Field label="Pays d’exercice" required value={form.ppe_pays_exercice} onChange={(v) => patchCurrent({ ppe_pays_exercice: v })} /><Field label="Période de la fonction" required value={form.ppe_anciennete} onChange={(v) => patchCurrent({ ppe_anciennete: v })} placeholder="Ex. depuis 2022 ou fin en mars 2026" /></div>}
