...
export default function ClientRecueilJourneyPage() {
  ...
  const validateSection = () => {
    ...
    if (current.code === 'professional') {
      ...
      if (professionalNeedsEmployer && (isBlank(form.societe) || isBlank(form.anciennete_annees))) throw new Error('Renseignez votre société / employeur et votre ancienneté.');
    ...
    if (current.code === 'objectives') {
      ...
      for (const item of form.items) {
        if (isBlank(item.montant_cible) || isBlank(item.horizon_annees) || Number(item.horizon_annees) < 0) throw new Error('Indiquez un montant cible et un horizon pour chaque objectif.');
  ...
  return <div>
    ...
    <WizardCard>
      ...
      <div className="space-y-6 px-6 py-7 sm:px-9 sm:py-9">
        ...
        {current.code === 'capacity' && <div className="grid gap-4 sm:grid-cols-2"><MoneyField label="Revenus du travail estimés annuels (€)" required value={form.estimation_revenus_travail_annuels} onChange={(v) => patchCurrent({ estimation_revenus_travail_annuels: v })} /><MoneyField label="Revenus fonciers estimés annuels (€)" required value={form.estimation_revenus_fonciers_annuels} onChange={(v) => patchCurrent({ estimation_revenus_fonciers_annuels: v })} /><MoneyField label="Épargne de précaution cible (€)" required value={form.epargne_precaution_cible} onChange={(v) => patchCurrent({ epargne_precaution_cible: v })} /><MoneyField label="Capacité d’épargne mensuelle (€)" required value={form.capacite_epargne_mensuelle} onChange={(v) => patchCurrent({ capacite_epargne_mensuelle: v })} /><MoneyField label="Apport immobilier possible (€)" value={form.apport_immobilier_possible} onChange={(v) => patchCurrent({ apport_immobilier_possible: v })} /></div>}
        ...
        <SecureNote>Les champs marqués * sont obligatoires. Chaque partie est enregistrée et horodatée. Après validation finale du recueil, vos réponses sont figées pour préserver la piste d’audit.</SecureNote>
      </div>
