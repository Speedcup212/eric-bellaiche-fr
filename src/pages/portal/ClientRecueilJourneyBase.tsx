...
const sections: Array<{ code: SectionCode; label: string; title: string; description: string }> = [
  ...
  { code: 'tax', label: 'Fiscalité', title: 'Situation fiscale', description: 'Les principales données fiscales peuvent être préremplies à partir de votre avis d’imposition. Vérifiez les valeurs et complétez uniquement ce qui manque.' },
...
const cabinetPreviewProgress: PortalProgress = {
  ...
  dossier_members_ready: 0, dossier_ready_for_documents: false, is_couple: false, partner_activated: false, partner_name: null, next_step: 'RECUEIL',
};
...
export default function ClientRecueilJourneyPage({ cabinetPreview = false }: { cabinetPreview?: boolean }) {
  ...
  const familyNeedsMatrimonialAdvantage = familySituation.includes('mari');
  ...
  return <div>
    ...
    <WizardCard>
      ...
      <div className="space-y-10 px-6 py-9 sm:px-9 sm:py-12">
        ...
        {current.code === 'family' && <div className="space-y-5">
          ...
          <section className="border-t border-white/10 pt-5">
            ...
            {(Array.isArray(form.enfants) ? form.enfants.length : 0) > 0 && <div className="mt-4 space-y-2">
              {(Array.isArray(form.enfants) ? form.enfants : []).map((child: AnyPayload, index: number) => <div key={index} className="grid items-end gap-3 border-t border-white/10 pt-3 first:border-t-0 first:pt-0 sm:grid-cols-[5rem_minmax(0,1fr)_minmax(0,1fr)_10rem_auto]">
        ...
        {current.code === 'professional' && <div className="grid gap-6 lg:grid-cols-[minmax(0,1.55fr)_minmax(280px,0.85fr)] lg:gap-x-6">
          <div className="space-y-5">
            ...
            <div className="w-full [&>div]:!border-0 [&>div]:!bg-transparent [&>div]:!p-0 [&>div]:!shadow-none"><MonthYearField required={professionalNeedsEmployer} value={String(form.date_entree ?? '')} onChange={(v) => patchCurrent({ date_entree: v })} /></div>
