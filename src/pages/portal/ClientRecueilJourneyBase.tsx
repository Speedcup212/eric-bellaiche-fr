...
function MonthYearField({ label = 'Date d’entrée dans l’entreprise', value, onChange, required = false, minYear = 1940 }: { label?: string; value: string; onChange: (value: string) => void; required?: boolean; minYear?: number }) {
  ...
  return <div className="rounded-2xl bg-[#0b1f3a] p-3 text-sm font-semibold text-white shadow-sm">
    ...
    <div className="mt-3 grid grid-cols-[1fr_auto_1fr] items-end gap-2 rounded-xl bg-white p-3 text-slate-700">
      <label className="!bg-transparent !p-0 !text-slate-700 !shadow-none"><span className="text-xs font-medium">Mois</span><select aria-label={`Mois — ${label}`} value={monthValue} onChange={(event) => commit(event.target.value, yearValue)} className="mt-1 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-3 text-sm font-normal text-slate-800 outline-none focus:border-[#6f8fb4] focus:bg-white"><option value="">MM</option>{months.map((month) => <option key={month} value={month}>{month}</option>)}</select></label>
      <span className="pb-3 text-lg font-semibold text-slate-500">/</span>
      <label className="!bg-transparent !p-0 !text-slate-700 !shadow-none"><span className="text-xs font-medium">Année</span><select aria-label={`Année — ${label}`} value={yearValue} onChange={(event) => commit(monthValue, event.target.value)} className="mt-1 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-3 text-sm font-normal text-slate-800 outline-none focus:border-[#6f8fb4] focus:bg-white"><option value="">AAAA</option>{years.map((year) => <option key={year} value={year}>{year}</option>)}</select></label>
    </div>
  </div>;
}
