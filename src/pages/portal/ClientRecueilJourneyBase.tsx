  ...
  return <div>
    ...
    <WizardCard>
      ...
      <div className="space-y-8 px-6 py-8 sm:px-9 sm:py-10">
        ...
        {current.code === 'regulatory' && <div className="space-y-6">
          ...
          {form.ppe_declaree === true && <div className="grid gap-4 sm:grid-cols-2"><Field label="Personne concernée" required value={form.ppe_personne_exposee} onChange={(v) => patchCurrent({ ppe_personne_exposee: v })} placeholder="Vous-même ou nom du proche" /><Field label="Fonction exercée" required value={form.ppe_motif} onChange={(v) => patchCurrent({ ppe_motif: v })} placeholder="Ex. député, ambassadeur, dirigeant public…" /><Field label="Pays d’exercice" required value={form.ppe_pays_exercice} onChange={(v) => patchCurrent({ ppe_pays_exercice: v })} /><Field label="Période de la fonction" required value={form.ppe_anciennete} onChange={(v) => patchCurrent({ ppe_anciennete: v })} placeholder="Ex. depuis 2022 ou fin en mars 2026" /></div>}
        ...
        {current.code === 'patrimony' && <div className="space-y-8">
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><h3 className="font-semibold text-[#0b1f3a]">Comptes courants et trésorerie disponible</h3><p className="mt-1.5 max-w-3xl text-sm leading-6 text-slate-600">Indiquez le solde réellement disponible sur vos comptes courants, puis la part que vous acceptez de mobiliser pour vos projets. Le solde total n’est pas considéré automatiquement comme investissable.</p></div><button type="button" onClick={() => patchCurrent({ comptes_courants: [...currentAccounts, { banque: '', titulaire: '', solde_actuel: '', montant_mobilisable: '', commentaire: '' }] })} className="inline-flex min-h-11 shrink-0 items-center justify-center gap-1 rounded-xl border border-blue-200 bg-blue-50 px-4 py-2.5 text-sm font-semibold text-[#173967] transition hover:border-[#3B82F6] hover:bg-blue-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/30"><Plus className="h-4 w-4" /> Ajouter un compte</button></div>{currentAccounts.length === 0 && <p className="mt-4 rounded-xl bg-slate-50 px-4 py-3 text-sm text-slate-600">Aucun compte courant déclaré.</p>}{currentAccounts.map((item: AnyPayload, index: number) => { const balance = parseAmount(item.solde_actuel); const mobilisable = parseAmount(item.montant_mobilisable); const keep = Math.max(balance - mobilisable, 0); const invalidMobilisable = item.montant_mobilisable !== '' && item.solde_actuel !== '' && mobilisable > balance; return <div key={index} className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-5"><div className="grid gap-4 sm:grid-cols-2"><Field label="Établissement bancaire" required value={item.banque} onChange={(v) => updateList('comptes_courants', index, { banque: v })} /><Field label="Titulaire / nature du compte" required value={item.titulaire} onChange={(v) => updateList('comptes_courants', index, { titulaire: v })} placeholder="Précisez si nécessaire" /><MoneyField label="Solde actuel (€)" required value={item.solde_actuel} onChange={(v) => updateList('comptes_courants', index, { solde_actuel: v })} /><MoneyField label="Montant que vous acceptez de mobiliser (€)" required value={item.montant_mobilisable} onChange={(v) => updateList('comptes_courants', index, { montant_mobilisable: v })} /></div><div className={`mt-4 rounded-xl px-4 py-3 text-sm ${invalidMobilisable ? 'bg-red-50 text-red-700' : 'bg-white text-slate-700'}`}>{invalidMobilisable ? 'Le montant mobilisable ne peut pas dépasser le solde du compte.' : <>Montant conservé sur ce compte après mobilisation : <strong>{formatAmount(keep)}</strong></>}</div><div className="mt-4"><Field label="Notes / précisions" value={item.commentaire} onChange={(v) => updateList('comptes_courants', index, { commentaire: v })} placeholder="Ex. somme réservée aux dépenses courantes, compte joint…" /></div><button type="button" onClick={() => removeList('comptes_courants', index)} className="mt-4 inline-flex items-center gap-1 text-xs font-semibold text-red-600"><Trash2 className="h-3.5 w-3.5" /> Supprimer</button></div>; })}{currentAccounts.length > 0 && <div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-xl bg-slate-50 p-4"><p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Soldes déclarés</p><p className="mt-1 text-lg font-semibold text-[#0b1f3a]">{formatAmount(totalCurrentAccounts)}</p></div><div className="rounded-xl bg-slate-50 p-4"><p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Montant mobilisable</p><p className="mt-1 text-lg font-semibold text-[#0b1f3a]">{formatAmount(totalMobilisable)}</p></div><div className="rounded-xl bg-slate-50 p-4"><p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Montant à conserver</p><p className="mt-1 text-lg font-semibold text-[#0b1f3a]">{formatAmount(totalToKeep)}</p></div></div>}</div>

          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><h3 className="font-semibold text-[#0b1f3a]">Patrimoine immobilier</h3><p className="mt-1.5 text-sm leading-6 text-slate-600">Ajoutez votre résidence principale, vos biens locatifs, résidences secondaires ou autres biens immobiliers.</p></div><button type="button" onClick={() => patchCurrent({ immobilier: [...(form.immobilier ?? []), { libelle: '', adresse: '', valeur_acquisition: '', valeur_actuelle: '', date_acquisition: '', dispositif_fiscal: '', loyer_hors_charges_mensuel: '', mode_detention: '', quote_part: '', usage_bien: '', commentaire: '' }] })} className="inline-flex min-h-11 shrink-0 items-center justify-center gap-1 rounded-xl border border-blue-200 bg-blue-50 px-4 py-2.5 text-sm font-semibold text-[#173967] transition hover:border-[#3B82F6] hover:bg-blue-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/30"><Plus className="h-4 w-4" /> Ajouter un bien</button></div>{(form.immobilier ?? []).length === 0 && <p className="mt-4 rounded-xl bg-slate-50 px-4 py-3 text-sm text-slate-600">Aucun bien immobilier déclaré.</p>}{(form.immobilier ?? []).map((item: AnyPayload, index: number) => <div key={index} className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-5"><div className="grid gap-4 sm:grid-cols-2"><Field label="Type / libellé" required value={item.libelle} onChange={(v) => updateList('immobilier', index, { libelle: v })} /><Field label="Adresse" required value={item.adresse} onChange={(v) => updateList('immobilier', index, { adresse: v })} /><MoneyField label="Valeur actuelle (€)" required value={item.valeur_actuelle} onChange={(v) => updateList('immobilier', index, { valeur_actuelle: v })} /><MoneyField label="Valeur d’acquisition (€)" value={item.valeur_acquisition} onChange={(v) => updateList('immobilier', index, { valeur_acquisition: v })} /><Field label="Usage" required value={item.usage_bien} onChange={(v) => updateList('immobilier', index, { usage_bien: v })} /><Field label="Mode de détention" required value={item.mode_detention} onChange={(v) => updateList('immobilier', index, { mode_detention: v })} /><MoneyField label="Loyer mensuel HC (€)" value={item.loyer_hors_charges_mensuel} onChange={(v) => updateList('immobilier', index, { loyer_hors_charges_mensuel: v })} /></div><button type="button" onClick={() => removeList('immobilier', index)} className="mt-4 inline-flex items-center gap-1 text-xs font-semibold text-red-600"><Trash2 className="h-3.5 w-3.5" /> Supprimer</button></div>)}</div>

          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><h3 className="font-semibold text-[#0b1f3a]">Placements financiers</h3><p className="mt-1.5 text-sm leading-6 text-slate-600">Livrets, PEA, assurance-vie, PER et autres placements. Les comptes courants sont renseignés séparément ci-dessus.</p></div><button type="button" onClick={() => patchCurrent({ placements: [...(form.placements ?? []), { type_contrat: '', organisme: '', libelle_contrat: '', montant_actuel: '', annee_ouverture: '', versements_programmes_annuels: '', montant_reemploi_possible: '', numero_contrat: '', montant_investi_avant_70_ans: '', montant_investi_apres_70_ans: '' }] })} className="inline-flex min-h-11 shrink-0 items-center justify-center gap-1 rounded-xl border border-blue-200 bg-blue-50 px-4 py-2.5 text-sm font-semibold text-[#173967] transition hover:border-[#3B82F6] hover:bg-blue-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/30"><Plus className="h-4 w-4" /> Ajouter un placement</button></div>{(form.placements ?? []).length === 0 && <p className="mt-4 rounded-xl bg-slate-50 px-4 py-3 text-sm text-slate-600">Aucun placement financier déclaré.</p>}{(form.placements ?? []).map((item: AnyPayload, index: number) => <div key={index} className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-5"><div className="grid gap-4 sm:grid-cols-2"><Field label="Type de contrat" required value={item.type_contrat} onChange={(v) => updateList('placements', index, { type_contrat: v })} placeholder="Autre placement" /><Field label="Organisme" required value={item.organisme} onChange={(v) => updateList('placements', index, { organisme: v })} /><MoneyField label="Montant actuel (€)" required value={item.montant_actuel} onChange={(v) => updateList('placements', index, { montant_actuel: v })} /><Field label="Année d’ouverture" type="number" value={item.annee_ouverture} onChange={(v) => updateList('placements', index, { annee_ouverture: v })} /><MoneyField label="Versements programmés annuels (€)" value={item.versements_programmes_annuels} onChange={(v) => updateList('placements', index, { versements_programmes_annuels: v })} /><MoneyField label="Montant de réemploi possible (€)" required value={item.montant_reemploi_possible} onChange={(v) => updateList('placements', index, { montant_reemploi_possible: v })} /></div><button type="button" onClick={() => removeList('placements', index)} className="mt-4 inline-flex items-center gap-1 text-xs font-semibold text-red-600"><Trash2 className="h-3.5 w-3.5" /> Supprimer</button></div>)}</div>
        </div>}
