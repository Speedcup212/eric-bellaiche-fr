        ...
        {current.code === 'patrimony' && <div className="real-estate-section space-y-6">
          ...
          {form.has_real_estate === true && <div className="space-y-3">
            ...
            {(form.immobilier ?? []).map((item: AnyPayload, index: number) => {
              ...
              <summary className="real-estate-card-header flex cursor-pointer list-none items-center justify-between gap-4 py-3.5"><div className="flex min-w-0 items-center gap-2.5"><Building2 className="h-4 w-4 shrink-0 text-[#60A5FA]" /><h4 className="truncate text-sm font-semibold text-white">{summary || `Bien immobilier ${index + 1}`}</h4></div><span className="flex shrink-0 items-center gap-1.5 text-xs font-semibold text-[#86B7FF]">{complete ? 'Modifier' : 'À compléter'}<ChevronDown className="h-4 w-4 transition-transform group-open:rotate-180" /></span></summary>
