  ...
  return <div>
    ...
    <WizardCard>
      ...
      <div className="space-y-10 px-6 py-9 sm:px-9 sm:py-12">
        ...
        {current.code === 'patrimony' && <div className="real-estate-section space-y-6 rounded-[24px] bg-[#f7f9fc] p-5 sm:p-7">
          <p className="real-estate-note flex items-center gap-2 text-xs leading-5 text-[#5b6b82]"><CheckCircle2 className="h-4 w-4 shrink-0 text-emerald-600" /><span><strong className="font-semibold text-[#33465f]">Aucun justificatif maintenant.</strong> Les prêts seront demandés dans « Documents ».</span></p>
        ...
        {current.code === 'financial' && <div className="financial-section space-y-6 rounded-[24px] bg-[#f7f9fc] p-5 sm:p-7">
          <p className="financial-note flex items-center gap-2 text-xs leading-5 text-[#5b6b82]"><CheckCircle2 className="h-4 w-4 shrink-0 text-emerald-600" /><span><strong className="font-semibold text-[#33465f]">Une estimation globale suffit.</strong> Les relevés préciseront ensuite les établissements et les montants.</span></p>
          <section>
            <h3 className="text-sm font-semibold text-slate-700">Quels comptes ou placements détenez-vous ? *</h3>
            <p className="mt-1 text-xs leading-5 text-[#64748b]">Plusieurs choix possibles.</p>
            {progress.is_couple && <p className="financial-couple-note mt-3 rounded-xl border border-blue-100 bg-blue-50 px-3.5 py-2.5 text-xs leading-5 text-[#3f5f82]">{progress.role_dossier === 'investisseur_1' ? 'Ajoutez vos avoirs personnels et les comptes communs, une seule fois.' : 'Ajoutez uniquement vos avoirs personnels.'}</p>}
            <div className="mt-4 grid gap-2.5 sm:grid-cols-2 lg:grid-cols-3">{financialCategoryOptions.map(([code, label]) => {
              ...
              }} className={`financial-choice min-h-12 rounded-xl border px-3.5 py-2.5 text-left text-sm font-semibold transition duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/30 ${selected ? 'financial-choice--selected border-[#3B82F6] bg-[#3B82F6] text-white' : 'border-[#dbe4ef] bg-white text-slate-700 hover:border-[#6ea8f8]'}`}><span className={`mr-2 inline-flex h-4 w-4 items-center justify-center rounded border align-[-2px] text-[10px] ${selected ? 'border-white/50 bg-white text-[#2563EB]' : 'border-[#aebdce] bg-white'}`}>{selected ? '✓' : ''}</span>{label}</button>;
            })}</div>
          </section>
          ...
          {(form.categories ?? []).length > 0 && !(form.categories ?? []).includes('none') && <fieldset>
            ...
            <p className="mt-1 text-xs leading-5 text-[#64748b]">Choisissez simplement une fourchette.</p>
            <div className="mt-4 grid gap-2.5 sm:grid-cols-2 lg:grid-cols-3">{financialTotalBands.map(([code, label]) => <button key={code} type="button" aria-pressed={form.total_band === code} onClick={() => patchCurrent({ total_band: code, completeness_confirmed: false })} className={`financial-band min-h-11 rounded-xl border px-3.5 py-2.5 text-sm font-semibold transition duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/30 ${form.total_band === code ? 'financial-band--selected border-[#3B82F6] bg-[#3B82F6] text-white' : 'border-[#dbe4ef] bg-white text-slate-700 hover:border-[#6ea8f8]'}`}>{label}</button>)}</div>
          </fieldset>}
          ...
          {(form.categories ?? []).length > 0 && <button type="button" aria-pressed={form.completeness_confirmed === true} onClick={() => patchCurrent({ completeness_confirmed: form.completeness_confirmed !== true })} className={`financial-confirm flex w-full items-start gap-3 rounded-xl border p-3.5 text-left text-sm leading-5 transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/30 ${form.completeness_confirmed === true ? 'border-emerald-300 bg-emerald-50 text-emerald-900' : 'border-[#dbe4ef] bg-white text-slate-700 hover:border-[#6ea8f8]'}`}><span className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded border ${form.completeness_confirmed === true ? 'border-emerald-600 bg-emerald-600 text-white' : 'border-[#aebdce] bg-white'}`}>{form.completeness_confirmed === true ? '✓' : ''}</span><span><strong>Je confirme avoir déclaré tous mes comptes et placements.</strong><br /><span className="text-xs font-normal text-slate-500">Les relevés permettront de vérifier et compléter les montants.</span></span></button>}
        </div>}
        ...
        {current.code === 'patrimony' || current.code === 'financial' ? <div className="rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-xs leading-5 text-[#CBD5E1]">Vos informations sont enregistrées de manière sécurisée.</div> : <RecueilInfoNote title="Enregistrement et traçabilité"><p>Les champs marqués * sont obligatoires. Les informations fiscales et les crédits seront renseignés à partir des justificatifs transmis en fin de parcours.</p><p className="mt-1.5 text-[#aebfd4]">Chaque partie est enregistrée et horodatée. Après validation finale, vos réponses sont figées afin de préserver la piste d’audit.</p></RecueilInfoNote>}
      </div>
