...
type InviteState = 'ready' | 'used' | 'expired' | 'invalid';
...
export default function ClientInvitationPage() {
  ...
  useEffect(() => {
    ...
    const initialize = async () => {
      ...
      if (status === 'expired') {
        ...
        setErrorMessage('Ce lien d’activation a expiré. Demandez au cabinet de vous transmettre un nouveau lien.');
        return;
      }
      if (status === 'invalid' || !data?.email) {
        localStorage.removeItem('cgp_pending_invite_token');
        setInviteState('invalid');
        setErrorMessage('Ce lien d’activation n’est pas valide.');
        return;
      }
  ...
  if (!token) {
    return <div className="relative min-h-screen bg-[#07111f] px-4 py-12 flex items-center justify-center"><div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_20%_0%,rgba(6,182,212,.2),transparent_35%),radial-gradient(circle_at_80%_10%,rgba(79,70,229,.2),transparent_35%)]" /><div className="relative w-full max-w-lg rounded-[30px] border border-white/10 bg-white/95 p-8 shadow-2xl"><ShieldCheck className="h-8 w-8 text-slate-400" /><h1 className="mt-5 text-2xl font-semibold">Lien personnel manquant</h1><p className="mt-3 text-sm leading-6 text-slate-500">Utilisez le lien sécurisé transmis par le cabinet pour activer votre dossier.</p><Link to="/espace-client/connexion" className="mt-6 inline-block font-semibold text-slate-900 underline">Accéder à mon espace client</Link></div></div>;
  }

  if (inviteState === 'used') {
    return (
      <div className="relative min-h-screen overflow-hidden bg-[#07111f] px-4 py-12 flex items-center justify-center">
        <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_20%_0%,rgba(6,182,212,.2),transparent_35%),radial-gradient(circle_at_80%_10%,rgba(79,70,229,.2),transparent_35%)]" />
        <div className="relative w-full max-w-lg rounded-[30px] border border-white/10 bg-white/95 p-8 shadow-2xl">
          <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-emerald-50 text-emerald-700"><ShieldCheck className="h-6 w-6" /></div>
          <p className="mt-6 text-xs font-bold uppercase tracking-[0.18em] text-slate-400">Cabinet Eric Bellaiche</p>
          <h1 className="mt-2 text-2xl font-semibold text-slate-950">Votre espace client est déjà activé</h1>
          <p className="mt-3 text-sm leading-6 text-slate-500">Ce lien servait uniquement à créer votre accès lors de la première connexion. Pour revenir à votre dossier, utilisez désormais votre adresse e-mail et le mot de passe que vous avez créé.</p>
          <Link to="/espace-client/connexion" onClick={resumeActivatedDossier} className="mt-6 flex w-full items-center justify-center rounded-2xl bg-slate-950 px-4 py-3.5 text-sm font-semibold text-white shadow-lg shadow-slate-950/15">Accéder à mon espace client</Link>
          <p className="mt-5 text-center text-xs leading-5 text-slate-500">Pour vos prochaines connexions, rendez-vous sur eric-bellaiche.fr et cliquez sur <strong>« Espace client »</strong> en haut du site.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="relative mx-auto grid min-h-[calc(100vh-5rem)] max-w-5xl items-center gap-10 lg:grid-cols-[1.05fr_.95fr]">
        ...
        <div className="mx-auto w-full max-w-md rounded-[30px] border border-white/10 bg-white/95 p-6 shadow-2xl shadow-black/30 backdrop-blur-xl sm:p-8">
          ...
          <form onSubmit={submit} className="mt-7 space-y-5">
            ...
            <div className="rounded-2xl bg-slate-50 px-4 py-3 text-xs leading-5 text-slate-500">Ce mot de passe vous permettra ensuite de revenir à votre dossier depuis le bouton <strong>« Espace client »</strong> sur eric-bellaiche.fr, sans rechercher votre e-mail d’invitation.</div>
          ...
          <p className="mt-6 text-center text-xs leading-5 text-slate-400">Dossier déjà activé ? <Link to="/espace-client/connexion" onClick={resumeActivatedDossier} className="font-semibold text-slate-700 underline">Accéder à mon espace client</Link></p>
        </div>
      </div>
    </div>
  );
