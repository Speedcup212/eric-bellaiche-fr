...
export default function CifAdminPage() {
  ...
  return <div className="min-h-screen bg-slate-50 px-4 py-10 sm:px-6"><div className="mx-auto max-w-6xl space-y-8">
    ...
    <section className="rounded-3xl border border-slate-200 bg-white p-6 sm:p-8"><h2 className="text-xl font-semibold">Dossiers</h2><div className="mt-5 divide-y divide-slate-100">{rows.map((row) => <div key={row.id} className="flex flex-col gap-3 py-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-semibold">{row.reference || row.libelle}</p><p className="text-xs text-slate-500">Recueil : {row.recueil_status} · Dossier : {row.statut}</p></div><div className="flex flex-wrap gap-2"><a href={`/cabinet/synthese?dossier=${encodeURIComponent(row.id)}`} className="inline-flex items-center gap-2 rounded-lg bg-slate-900 px-3 py-2 text-sm font-semibold text-white">Synthèse conseiller</a><button onClick={() => void listInvestors(row.id)} className="inline-flex items-center gap-2 rounded-lg border border-slate-300 px-3 py-2 text-sm font-semibold"><Mail className="h-4 w-4" /> Préparer l’invitation</button></div></div>)}</div></section>
  </div></div>;
}
