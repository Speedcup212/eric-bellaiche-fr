...
begin
  ...
  select bool_or(dca.tax_status='personal_notice'), bool_or(dca.has_liquidities), bool_or(dca.has_financial_assets), bool_or(dca.has_real_estate), bool_or(dca.has_credits), bool_or(dca.has_sci_company)
  ...
  if coalesce(v_required_liquidities,false) and not exists(select 1 from public.documents_sources ds where ds.dossier_id=p_dossier_id and ds.categorie='comptes_liquidites') then raise exception 'Ajoutez un relevé du ou des comptes courants.'; end if;
  ...
  select jsonb_build_object('dossier_id',p_dossier_id,'members_total',v_total,'members_ready',v_ready,'document_context_ready',v_context_ready,'documents_count',v_count,'required',jsonb_build_object('identity_count',v_total,'domicile',true,'tax',coalesce(v_required_tax,false),'liquidities',coalesce(v_required_liquidities,false),'financial_assets',coalesce(v_required_assets,false),'real_estate',coalesce(v_required_real_estate,false),'credits',coalesce(v_required_credits,false),'sci_company',coalesce(v_required_sci,false)),'transmitted_at',v_now)
  into v_progress;
