...
begin
  ...
  from public.questionnaire_answers qa
  join public.questionnaire_questions qq on qq.id = qa.question_id
  join public.questionnaire_options qo on qo.id = qa.option_id
  where qa.session_id = new.session_id
    and qq.code = 'Q10'
  limit 1;
  ...
  v_capacity_rank := case v_q10_code
    when 'A' then 1
    when 'B' then 2
    when 'C' then 3
    when 'D' then 4
  ...
  if v_scored_count <> 5 or v_capacity_count <> 4 or v_tolerance_rank not between 1 and 7 or v_capacity_rank not between 1 and 7 then
    ...
    new.synthese_dimensions := jsonb_set(
      coalesce(new.synthese_dimensions, '{}'::jsonb),
      '{profil_operationnel}',
      jsonb_build_object('rang', null, 'profil', null),
      true
    );
    return new;
  end if;

  v_capacity_profile := case v_capacity_rank
  ...
  new.synthese_dimensions := jsonb_set(
    coalesce(new.synthese_dimensions, '{}'::jsonb),
    '{capacite_perte}',
    coalesce(new.synthese_dimensions->'capacite_perte', '{}'::jsonb)
      || jsonb_build_object('rang', v_capacity_rank, 'profil_plafond', v_capacity_profile),
    true
  );
  ...
  new.justification_ecart := case
    when v_tolerance_rank > v_capacity_rank
      then 'Le profil a été limité par la perte financière maximale que le client déclare pouvoir supporter.'
    else null
  end;
  new.synthese_dimensions := jsonb_set(
    coalesce(new.synthese_dimensions, '{}'::jsonb),
    '{profil_operationnel}',
    jsonb_build_object('rang', v_expected_rank, 'profil', v_expected_profile),
    true
  );
  return new;
end;
