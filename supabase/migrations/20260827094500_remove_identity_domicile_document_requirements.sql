...
begin
  ...
  select count(*), count(*) filter(
    where di.recueil_status='validated'
      and di.qpi_status in ('completed','validated')
      and (di.esg_opt_in is not true or di.esg_status in ('completed','validated'))
  )
  ...
  if coalesce(v_required_tax,false)
     and not exists(select 1 from public.documents_sources ds where ds.dossier_id=p_dossier_id and ds.categorie='avis_imposition') then
    raise exception 'Ajoutez l’avis d’imposition indiqué comme disponible.';
  end if;

  if coalesce(v_required_assets,false)
     and not exists(select 1 from public.documents_sources ds where ds.dossier_id=p_dossier_id and ds.categorie='patrimoine_financier') then
    raise exception 'Ajoutez un relevé d’épargne ou de placement.';
  end if;

  if coalesce(v_required_real_estate,false)
     and not exists(select 1 from public.documents_sources ds where ds.dossier_id=p_dossier_id and ds.categorie='patrimoine_immobilier') then
    raise exception 'Ajoutez un justificatif de patrimoine immobilier.';
  end if;

  if coalesce(v_required_credits,false)
     and not exists(select 1 from public.documents_sources ds where ds.dossier_id=p_dossier_id and ds.categorie='tableau_amortissement') then
    raise exception 'Ajoutez le tableau d’amortissement ou le justificatif du crédit en cours.';
  end if;

  if coalesce(v_required_sci,false)
     and not exists(select 1 from public.documents_sources ds where ds.dossier_id=p_dossier_id and ds.categorie='sci_societe') then
  ...
  set documents_status='completed',
      documents_completed_at=v_now,
      transmitted_at=v_now,
      transmission_snapshot=v_progress,
      updated_at=v_now
  where dossier_id=p_dossier_id;

  insert into public.validations(
    dossier_id,investisseur_id,objet_type,objet_id,type_validation,statut,methode,commentaire,valide_par,validated_at
  )
  values(
    p_dossier_id,v_investisseur_id,'dossier',p_dossier_id,'transmission_finale_dossier','accepted','parcours_securise',
    'Transmission finale du dossier commun après complétion de tous les parcours individuels et contrôle des justificatifs attendus',
    v_uid,v_now
  );
end;
$$;
