...
const VERSION = 'AUDIT-CLIENT-2026-09-20-2';
...
function signatureSection(c:Ctx,investors:J[]){ newPage(c); h(c,'Signatures'); text(c,'Le document est destiné à être signé électroniquement. La date, l’horodatage et les éléments de preuve sont portés par le prestataire de signature du cabinet.',{size:8.8,color:GREY,after:10}); const people=[...investors.map(i=>({name:invName(i),role:'Client'})),{name:'Eric Bellaiche',role:'Conseiller'}].slice(0,3); const gap=10, totalW=A4.width-2*M, boxW=(totalW-gap*(people.length-1))/people.length, boxH=145; ensure(c,boxH+10); people.forEach((p,i)=>{ const x=M+i*(boxW+gap); c.page.drawRectangle({x,y:c.y-boxH,width:boxW,height:boxH,borderWidth:.7,borderColor:BORDER,color:LIGHT}); c.page.drawText(p.name,{x:x+8,y:c.y-18,size:8.6,font:c.bold,color:BLUE}); c.page.drawText(p.role,{x:x+8,y:c.y-33,size:7.5,font:c.regular,color:GREY}); c.page.drawText('Signature électronique',{x:x+8,y:c.y-58,size:7.8,font:c.bold,color:DARK}); c.page.drawText('Date et horodatage apposés',{x:x+8,y:c.y-77,size:7.2,font:c.regular,color:DARK}); c.page.drawText('par le prestataire de signature',{x:x+8,y:c.y-93,size:7.2,font:c.regular,color:DARK}); }); c.y-=boxH+8; }
