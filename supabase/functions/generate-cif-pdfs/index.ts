...
async function buildRecueilCouple(snapshot: Json) {
  ...
  maps.forEach(({ inv, map }: Json, index: number) => {
    ...
    const documentedItems = items.filter((item: Json) => Boolean(item.source_document_id));
    const documentedTotal = documentedItems.reduce((sum: number, item: Json) => sum + num(item.montant ?? item.valeur ?? item.encours), 0);
    if (items.length) drawTable(ctx, ['Placement','Organisme','Titulaire','Montant','Source'], items.map((item: Json) => [
      clean(item.type_placement ?? item.type_contrat ?? item.type),
      clean(item.organisme ?? item.etablissement),
      clean(item.proprietaire ?? item.titulaire),
      eur(item.montant ?? item.valeur ?? item.encours),
      item.source_document_id ? 'Justificatif' : item.source === 'synthese_dossier' || item.source_type === 'synthese_dossier' ? 'Synthèse dossier' : 'Déclaré'
    ]), [28,20,18,17,17]);
    drawTable(ctx, ['Donnée','Valeur'], [
      ...
      ['Autres placements / précisions', clean(fin.other_details)],
      ['Complétude confirmée', clean(fin.completeness_confirmed)],
    ], [58,42]);
  });

  const crdTotal = credits.reduce((sum: number, x: Json) => sum + num(x.crd ?? x.capital_restant_du), 0);
  const monthlyDebt = credits.reduce((sum: number, x: Json) => sum + num(hasValue(x.mensualite_actuelle) ? x.mensualite_actuelle : x.mensualite), 0);
...
async function buildRecueil(snapshot: Json) {
  if (snapshot.investors?.length > 1) return buildRecueilCouple(snapshot);
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const payload = await req.json(); const dossierId = typeof payload?.dossier_id === 'string' ? payload.dossier_id : ''; if (!/^[0-9a-f-]{36}$/i.test(dossierId)) return new Response(JSON.stringify({ error: 'Dossier invalide' }), { status: 400, headers }); const targetInvestorId = typeof payload?.investisseur_id === 'string' ? payload.investisseur_id : ''; if (targetInvestorId && !/^[0-9a-f-]{36}$/i.test(targetInvestorId)) return new Response(JSON.stringify({ error: 'Investisseur invalide' }), { status: 400, headers }); const requested = Array.isArray(payload?.document_types) ? payload.document_types : ['recueil', 'qpi', 'esg']; const types = requested.filter((x: string): x is DocumentType => ['recueil', 'qpi', 'esg', 'der', 'mission'].includes(x)); if (!types.length) return new Response(JSON.stringify({ error: 'Aucun type de document demandé' }), { status: 400, headers }); const fullSnapshot = await loadSnapshot(userClient, dossierId); const investorSnapshot = targetInvestorId ? scopeSnapshotToInvestor(fullSnapshot, targetInvestorId) : fullSnapshot; const admin = createClient(supabaseUrl, serviceKey, { auth: { persistSession: false, autoRefreshToken: false } }); const results: Json[] = []; const generationErrors: Json[] = [];
    for (const type of types) {
      try {
        const householdRecueil = type === 'recueil' && fullSnapshot.investors.length > 1;
        const snapshot = householdRecueil ? fullSnapshot : investorSnapshot;
        const effectiveInvestorId = householdRecueil ? '' : targetInvestorId;
        validateReady(snapshot, type);
      const dbDocumentType = type === 'mission' ? 'lettre_mission' : type;
      const snapshotHash = await sha256Hex(JSON.stringify({ type, version: PDF_VERSION, dossier: snapshot.dossier, investor_id: effectiveInvestorId || null, investors: snapshot.investors, sections: snapshot.sections, sessions: snapshot.sessions, qpi: snapshot.qpiResults, esg: snapshot.esgPreferences, qpi_experience: snapshot.qpiProductExperience, qpi_experience_details: snapshot.qpiExperienceDetails, answers: snapshot.answers, recueil_completeness: snapshot.recueilCompleteness ?? [] })); const { data: existing } = await admin.from('documents_reglementaires').select('id,storage_bucket,storage_path_pdf,metadata,date_generation').eq('dossier_id', dossierId).eq('type_document', dbDocumentType).eq('version_modele', PDF_VERSION).eq('metadata->>snapshot_hash', snapshotHash).eq('statut', 'generated').order('created_at', { ascending: false }).limit(1).maybeSingle(); if (existing?.storage_path_pdf) { const { data: signed } = await admin.storage.from(existing.storage_bucket ?? BUCKET).createSignedUrl(existing.storage_path_pdf, 3600); results.push({ type, investisseur_id: effectiveInvestorId || null, format: 'pdf', document_id: existing.id, reused: true, signed_url: signed?.signedUrl ?? null, path: existing.storage_path_pdf }); continue; }
      const bytes = type === 'recueil' ? await buildRecueil(snapshot) : type === 'qpi' ? await buildQuestionnaire(snapshot, 'QPI') : type === 'esg' ? await buildQuestionnaire(snapshot, 'ESG') : type === 'der' ? await buildDer(snapshot) : await buildMission(snapshot); const fileHash = await sha256Hex(bytes); const datePart = new Date().toISOString().slice(0, 10); const reference = slug(snapshot.dossier.reference || snapshot.dossier.libelle || dossierId.slice(0, 8)); const investorSlug = effectiveInvestorId ? `-${slug(investorName(snapshot.investors[0]))}` : ''; const isRegulatoryHousehold = !effectiveInvestorId && (type === 'der' || type === 'mission'); const isHouseholdRecueil = type === 'recueil' && !effectiveInvestorId && snapshot.investors.length > 1; const fileName = isRegulatoryHousehold ? regulatoryPdfFileName(type as 'der' | 'mission', snapshot, datePart) : isHouseholdRecueil ? `recueil-informations-foyer-${reference}-${datePart}-${fileHash.slice(0, 10)}.pdf` : `${type}-${reference}${investorSlug}-${datePart}-${fileHash.slice(0, 10)}.pdf`; const storagePath = effectiveInvestorId ? `${dossierId}/${effectiveInvestorId}/${type}/${fileName}` : (isRegulatoryHousehold || isHouseholdRecueil) ? `${dossierId}/${type}/${fileHash.slice(0, 10)}/${fileName}` : `${dossierId}/${type}/${fileName}`; const { error: uploadError } = await admin.storage.from(BUCKET).upload(storagePath, bytes, { contentType: 'application/pdf', upsert: true }); if (uploadError) throw uploadError; const { data: row, error: insertError } = await admin.from('documents_reglementaires').insert({ dossier_id: dossierId, type_document: dbDocumentType, version_modele: PDF_VERSION, statut: 'generated', storage_bucket: BUCKET, storage_path_pdf: storagePath, date_generation: new Date().toISOString(), hash_sha256: fileHash, metadata: { snapshot_hash: snapshotHash, generated_from: 'portal_supabase_pdf', final_format: 'pdf', signature_provider: ['der','mission'].includes(type) ? 'youtrust_manual' : 'youtrust', signature_status: ['der','mission'].includes(type) ? 'pdf_ready_for_manual_upload' : type === 'recueil' && !((snapshot.recueilCompleteness ?? []).length > 0 && (snapshot.recueilCompleteness ?? []).every((row: Json) => row.complete === true)) ? 'draft' : 'ready_to_send', signature_scope: type === 'recueil' && snapshot.investors.length > 1 ? 'household_all_clients' : 'individual', recueil_complete: type === 'recueil' ? ((snapshot.recueilCompleteness ?? []).length > 0 && (snapshot.recueilCompleteness ?? []).every((row: Json) => row.complete === true)) : null, recueil_percentage: type === 'recueil' && (snapshot.recueilCompleteness ?? []).length ? Math.round((snapshot.recueilCompleteness ?? []).reduce((sum: number, row: Json) => sum + Number(row.percentage ?? 0), 0) / (snapshot.recueilCompleteness ?? []).length) : null, document_date: type === 'recueil' ? snapshot.recueil_date : type === 'qpi' ? snapshot.qpi_date : type === 'esg' ? snapshot.esg_date : new Date().toISOString(), investor_id: effectiveInvestorId || null, investor_ids: snapshot.investors.map((i: Json) => i.id), source_word_generator: 'generate-cif-documents' } }).select('id').single(); if (insertError) throw insertError; const { data: signed } = await admin.storage.from(BUCKET).createSignedUrl(storagePath, 3600); results.push({ type, investisseur_id: effectiveInvestorId || null, format: 'pdf', document_id: row.id, reused: false, signed_url: signed?.signedUrl ?? null, path: storagePath, hash_sha256: fileHash });
      } catch (documentError) {
        const message =
          documentError instanceof Error
            ? documentError.message
            : documentError && typeof documentError === 'object' && 'message' in documentError
              ? String((documentError as { message?: unknown }).message ?? 'Génération impossible')
              : typeof documentError === 'string'
                ? documentError
                : 'Génération impossible';
        console.error('generate-cif-pdfs document', { dossierId, targetInvestorId, type, message, documentError });
