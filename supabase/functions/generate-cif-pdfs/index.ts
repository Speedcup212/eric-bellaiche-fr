...
const PDF_VERSION = '2026-MAITRE-PDF-2.7';
...
async function buildRecueil(snapshot: Json) {
  ...
  heading(ctx, `${n++}. Patrimoine financier et liquidités`); for (const { inv, map } of maps) { const fin = map.financial ?? {}; const items = Array.isArray(fin.items) ? fin.items : []; drawText(ctx, investorName(inv), { bold: true, size: 9.5, color: BLUE, after: 5 }); if (items.length) { drawTable(ctx, ['Placement', 'Organisme', 'Titulaire', 'Montant'], items.map((item: Json) => [clean(item.type_placement ?? item.type_contrat ?? item.type), clean(item.organisme ?? item.etablissement), clean(item.proprietaire ?? item.titulaire), eur(item.montant ?? item.valeur ?? item.encours)]), [34, 24, 22, 20]); } drawTable(ctx, ['Donnée', 'Valeur'], [['Total financier exact documenté', items.length ? eur(items.reduce((sum: number, item: Json) => sum + num(item.montant ?? item.valeur ?? item.encours), 0)) : eur(fin.exact_total_amount)], ['Liquidités importantes volontairement conservées sur comptes courants', clean(fin.current_accounts_intentional)], ['Catégories de placements', financialCategoryLabel(fin.categories)], ['Fourchette déclarée', financialBandLabel(fin.total_band)], ['Autres placements / précisions', clean(fin.other_details)], ['Complétude confirmée', clean(fin.completeness_confirmed)]], [58, 42]); }
