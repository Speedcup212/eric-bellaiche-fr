...
const PDF_VERSION = '2026-MAITRE-PDF-2.6';
...
function pct(value: unknown) { return `${new Intl.NumberFormat('fr-FR', { maximumFractionDigits: 2 }).format(num(value))} %`; }
...
async function buildRecueil(snapshot: Json) {
  ...
  let n = 1; const properties: Json[] = []; const credits: Json[] = []; let incomeAnnual = 0; let financialExact = 0;
  for (const { inv, map } of maps) {
    ...
    const tax = map.tax ?? {}; heading(ctx, `${n++}. Situation fiscale`); drawTable(ctx, ['Donnée fiscale', 'Valeur'], [['Année d’imposition', clean(tax.annee_imposition)], ['Revenu imposable', eur(tax.revenu_imposable)], ['Revenu fiscal de référence', eur(tax.revenu_fiscal_reference)], ['Nombre de parts', clean(tax.nombre_parts)], ['TMI', pct(tax.tmi)], ['Impôt sur le revenu net', eur(tax.impot_revenu_net)], ['Salaires / assimilés', eur(tax.salaires_assimiles)], ['Revenus fonciers nets', eur(tax.revenus_fonciers_nets)], ['Déficit foncier reportable', eur(tax.deficit_foncier_reportable)], ['Plafond épargne retraite disponible', eur(tax.plafond_disponible_avis)], ['Versements retraite à déduire', eur(tax.versements_a_deduire)]], [58, 42]); if (tax.ifi_concerne === true) { heading(ctx, 'IFI', 2); drawTable(ctx, ['Donnée IFI', 'Valeur'], [['Base imposable IFI', eur(tax.ifi_base_imposable)], ['TMI IFI', pct(tax.ifi_tmi)], ['IFI net à payer', eur(tax.ifi_net_a_payer)]], [58, 42]); }
    if (map.patrimony?.has_real_estate === true) properties.push(...(map.patrimony?.immobilier ?? [])); if (map.credits?.has_credits === true) credits.push(...(map.credits?.items ?? [])); const placements = map.financial?.items ?? map.patrimony?.placements ?? []; financialExact += placements.reduce((sum: number, x: Json) => sum + num(x.montant ?? x.valeur ?? x.encours), 0);
  }
  ...
  heading(ctx, `${n++}. Patrimoine financier et liquidités`); for (const { inv, map } of maps) { const fin = map.financial ?? {}; drawText(ctx, investorName(inv), { bold: true, size: 9.5, color: BLUE, after: 5 }); drawTable(ctx, ['Donnée', 'Valeur'], [['Liquidités importantes volontairement conservées sur comptes courants', clean(fin.current_accounts_intentional)], ['Catégories de placements', clean(fin.categories)], ['Fourchette de patrimoine financier', clean(fin.total_band)], ['Autres placements / précisions', clean(fin.other_details)], ['Complétude confirmée', clean(fin.completeness_confirmed)]], [58, 42]); }
