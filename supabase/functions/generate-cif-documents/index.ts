...
function buildRecueil(snapshot: Json) {
  ...
  children.push(table(memberHeaders, compareRows([
    ...
    ['Personne exposée', (map) => map.regulatory?.ppe_declaree === false ? 'Non' : text(map.regulatory?.ppe_personne_exposee)],
    ['Fonction PPE', (map) => map.regulatory?.ppe_declaree === false ? 'Non' : text(map.regulatory?.ppe_motif)],
    ['Pays d’exercice PPE', (map) => map.regulatory?.ppe_declaree === false ? 'Non' : text(map.regulatory?.ppe_pays_exercice)],
    ['Période PPE', (map) => map.regulatory?.ppe_declaree === false ? 'Non' : text(map.regulatory?.ppe_anciennete)],
